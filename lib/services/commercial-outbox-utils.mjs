...
export function createDecisionEvent({ campaignKey, externalKey, decision, occurredAt }) {
  ...
  const entityVersion = Math.max(1, Math.floor(Date.parse(occurredAt)))
  const subject = `urn:geimser:campaign-lead:${sha256(campaignKey).slice(0, 16)}:${externalKey}`
...
export function validateDecisionOutboxRow(row) {
  ...
  return typeof campaignKey === 'string'
    ...
    && item.event_source === 'urn:geimser:bigdata'
    ...
    && typeof item.data_schema === 'string'
    && item.data_schema.trim().length > 0
    && item.tenant_id === 'geimser'
    && Number.isInteger(item.entity_version)
    && item.entity_version >= 1
    && typeof item.correlation_id === 'string'
    && item.correlation_id.trim().length > 0
    && (item.causation_id === null || typeof item.causation_id === 'string')
...
function transportBody(rows) {
  return JSON.stringify({
    schema_version: '2',
    campaign_key: rows[0]?.payload?.campaign_key,
    items: rows.map(row => row.payload.item),
  })
}
