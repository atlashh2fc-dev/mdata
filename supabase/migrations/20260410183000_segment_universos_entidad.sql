...
AS $$
  SELECT CASE
    WHEN regexp_replace(upper(COALESCE(p_rutid, '')), '[^0-9K]', '', 'g') IN ('', '0000000000')
      OR regexp_replace(upper(COALESCE(p_rutid, '')), '[^0-9K]', '', 'g') !~ '^[0-9]{9}[0-9K]$'
      THEN 'invalido'
    WHEN COALESCE(
...
GRANT SELECT ON public.personas_master_clasificada TO authenticated;
GRANT SELECT ON public.personas_master_clasificada TO anon;
...
GRANT SELECT ON public.stats_universos TO authenticated;
GRANT SELECT ON public.stats_universos TO anon;
